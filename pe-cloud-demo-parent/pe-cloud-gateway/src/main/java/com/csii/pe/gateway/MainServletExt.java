...
public class MainServletExt extends HttpServlet implements ApplicationContextAware {
	protected static String[] DEFAULT_ALLOWED_LOCALES = new String[]{"en_US", "zh_CN", "zh_TW"};
	protected Log logger = LogFactory.getLog(this.getClass());
	private boolean publishContext = true;
	public static final String SERVLET_CONTEXT_PREFIX = MainServletExt.class.getName() + ".CONTEXT.";
	...
	private void initMultipartResolver() throws BeansException {
		try {
			...
			if (this.logger.isInfoEnabled()) {
				this.logger.info(
						"Unable to locate MultipartResolver with name [multipartResolver]: no multipart handling provided");
			}
		}
	...
	private void initLocaleResolver() throws BeansException {
		try {
			...
			if (this.logger.isInfoEnabled()) {
				this.logger.info("Unable to locate LocaleResolver with name 'localeResolver': using default ["
						+ this.localeResolver + "]");
			}
		}
		...
		if (this.multipartResolver != null && this.multipartResolver.isMultipart(request)) {
			if (request instanceof MultipartHttpServletRequest) {
				...
				try {
					...
					if (map != null) {
						this.onError(request, response, var10, (String) map.get("ErrorPage"));
						return;
					}
		...
		try {
			...
			if (newLocale != null) {
				if (!this.allowedLocales.contains(newLocale)) {
					...
					throw new PeRuntimeException("invalid_locale",
							new Object[]{newLocale, this.allowedLocales.toString()});
				}
